...
class KnowledgeGraphBuilder:
    ...
    def build_base_graph(self) -> List:
        ...
        try:
            ...
            with self._create_progress() as progress:
                task = progress.add_task("[cyan]处理文件...", total=1)    #进度条总进度，task是任务id
                
                # 使用DocumentProcessor处理文件
                self.processed_documents = self.document_processor.process_directory()  #返回的是一个列表，列表中的每个元素是一个字典，字典中包含文件名、内容、分块等信息
